...
//
//sealed trait CalcM[+F[+_], -R, -S1, +S2, +O, +E, +A] extends SinkStep[F, Any, Any, Any, R, S1, S2, O, E, A]{
//  def narrowRead[R1 <: R]: CalcM[F, R1, S1, S2, O, E, A] = this
//  def mapK[G[+_]](fk: FunK[F, G]): CalcM[G, R, S1, S2, O, E, A]
//}
//
//object CalcM {
...
//    type R1 = R
//    def any                                                                  = Is.refl
//    def mapK[G[+_]](fk: FunK[F, G]): CalcM[G, Any, S1, S2, O, E, A] = Provide(r, inner.mapK(fk))
//  }
//
...
//          def output(out: O1): SinkStep[A, O1, E1, CalcM[G, R, S2, S3, O2, E2, B]] =
...
//  sealed trait SinkStep[+F[+_], -XO, -XE, -XA, -R, -S1, +S2, +O, +E, +A]{
//    def mapK[G[+_]](f: FunK[F, G]): SinkStep[G, XO, XE, XA, S1, S2, O, E, A]
//  }
//
//  trait Continue[+F[+_], -XO, -XE, -XA, -R, -S1, +S2, +O, +E, +A] extends SinkStep[F, XO, XE, XA, R, S1, S2, O, E, A] {
//    self =>
//    def success(result: XA): CalcM[F, R, S1, S2, O, E, A]
//    def error(err: XE): CalcM[F, R, S1, S2, O, E, A]
//    def output(out: XO): CalcM[F, R, S1, S2, O, E, SinkStep[F, XO, XE, XA, R, S1, S2, O, E, A]]
//  }
//
//  object Continue {
...
//        def output(out: O): SinkStep[A, O, E, X] = this
//      }
//
...
//        def output(out: O): SinkStep[A, O, Nothing, X] = this
//      }
//
...
//        def error(err: E): X                           = f(err)
//        def output(out: O): SinkStep[Nothing, O, E, X] = this
//      }
//  }
//
